...
CREATE TABLE IF NOT EXISTS workshop_review (
  id text PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS visit_evidence (
  id text PRIMARY KEY,
  review_id text NOT NULL UNIQUE REFERENCES workshop_review(id),
...
CREATE TABLE IF NOT EXISTS review_update (
  id text PRIMARY KEY,
  review_id text NOT NULL REFERENCES workshop_review(id),
  author_user_id text NOT NULL REFERENCES app_user(id),
...
CREATE INDEX IF NOT EXISTS workshop_review_public_idx
  ON workshop_review (workshop_id, visit_month DESC, submitted_at DESC)
  WHERE publication_state = 'published';
CREATE INDEX IF NOT EXISTS review_update_review_idx ON review_update (review_id, created_at);

ALTER TABLE workshop_review ENABLE ROW LEVEL SECURITY;
ALTER TABLE visit_evidence ENABLE ROW LEVEL SECURITY;
ALTER TABLE review_moderator_assignment ENABLE ROW LEVEL SECURITY;
ALTER TABLE review_workshop_response ENABLE ROW LEVEL SECURITY;
ALTER TABLE review_update ENABLE ROW LEVEL SECURITY;

CREATE POLICY workshop_review_author_or_moderator ON workshop_review
  USING (
    ...
    OR EXISTS (
      ...
      WHERE review_moderator_assignment.review_id = workshop_review.id
...
CREATE POLICY review_response_active_member_or_admin ON review_workshop_response
  USING (
    current_setting('app.system_role', true) = 'admin'
    OR EXISTS (
      SELECT 1 FROM membership
      WHERE membership.workshop_id = review_workshop_response.workshop_id
...
CREATE OR REPLACE VIEW public_workshop_review AS
SELECT
  review.id,
  review.workshop_id,
...
CREATE OR REPLACE VIEW public_workshop_review_summary AS
SELECT
  workshop_id,
...
CREATE OR REPLACE VIEW public_review_workshop_response AS
SELECT
  response.review_id,
  response.response_text,
  response.created_at
FROM review_workshop_response AS response
JOIN public_workshop_review AS review ON review.id = response.review_id;
...
JOIN public_workshop_review AS review ON review.id = update_entry.review_id;
