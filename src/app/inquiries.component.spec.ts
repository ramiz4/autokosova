...
const identity: OwnAccount = { userId: 'fixture-owner', displayName: 'Fiktives Konto', roles: ['customer', 'admin'], garageMemberships: [], expiresAt: new Date(Date.now() + 3600_000).toISOString() };
const detail: SavedRepairRequest = {
  id: 'fixture-inquiry', serviceCategoryId: 'bremsen', symptom: '<b>PRIVATE-SYMPTOM</b>',
  createdAt: '2026-09-14T12:00:00Z', areas: [{ placeId: 'xk-pristina', radiusKm: 20 }],
  vehicle: { makeId: 'skoda', model: '<img src=x>', year: 2018, engineDetails: 'PRIVATE-ENGINE', mileageKm: 0 },
  earliestDropoffOn: '2026-10-02', latestPickupOn: '2026-10-06', attachmentIds: ['PRIVATE-FILE-ID'],
};
const pageData: RepairRequestPage = { requests: [{ id: detail.id, serviceCategoryId: detail.serviceCategoryId,
  symptomPreview: detail.symptom, createdAt: detail.createdAt, areas: detail.areas,
  vehicle: { makeId: 'skoda', model: 'Fixture' } }], nextCursor: null };
...
beforeEach(() => {
  ...
  vi.stubGlobal('fetch', vi.fn().mockImplementation(async (url: RequestInfo | URL) => {
    ...
    if (path === '/auth/logout') { current = null; return new Response(null, { status: 204 }); }
    if (path === '/api/me/repair-requests') return listResponse();
    if (path.startsWith('/api/me/repair-requests/')) return detailResponse();
    return json({}, 404);
  }));
});
afterEach(() => { sessionStorage.removeItem('autokosova.repair-request-draft.v1'); vi.unstubAllGlobals(); });

async function render(path = '/inquiries') {
  await TestBed.configureTestingModule({ imports: [InquiriesComponent], providers: [provideRouter(routes)] }).compileComponents();
  await TestBed.inject(Router).navigateByUrl(path);
  TestBed.inject(RepairRequestDraft).write({ symptom: 'UNSAVED-DRAFT', serviceCategoryId: 'motor' });
...
it.each(['de', 'sq', 'en'] as const)('uses the canonical %s overview, readonly details and only approved search filters', async (locale) => {
  ...
  expect(page.querySelector('[data-new-inquiry]')?.getAttribute('href')).toBe(routePath(locale, 'request'));
  ...
  expect(href).not.toContain('PRIVATE'); expect(href).not.toContain(detail.id);
  page.querySelector<HTMLButtonElement>('[data-inquiry-view]')!.click();
  await vi.waitFor(() => expect(service.detailState()).toBe('ready')); await fixture.whenStable();
  ...
  expect(page.querySelector('[data-inquiry-detail] input, [data-inquiry-detail] textarea')).toBeNull();
  ...
  expect(TestBed.inject(RepairRequestDraft).read()).toEqual({ symptom: 'UNSAVED-DRAFT', serviceCategoryId: 'motor' });
...
it('links only the signed-in account menu, marks the active route, preserves profile/roles and closes with keyboard or selection', async () => {
  ...
  await vi.waitFor(() => expect(account.state()).toBe('ready')); await fixture.whenStable();
  ...
  link.click(); await fixture.whenStable(); expect(page.querySelector('#account-menu')).toBeNull();
  toggle.click(); await vi.waitFor(() => expect(account.state()).toBe('ready')); await fixture.whenStable();
  toggle.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true }));
  await fixture.whenStable(); expect(page.querySelector('#account-menu')).toBeNull();
  expect(document.activeElement).toBe(toggle);
  current = null; account.invalidate(); await fixture.whenStable();
  toggle.click(); await vi.waitFor(() => expect(account.state()).toBe('guest')); await fixture.whenStable();
...
it('distinguishes empty, retry, missing detail and expired session without reading a local draft as saved', async () => {
  ...
  expect(page.querySelector('main [role="alert"]')?.textContent).toContain(inquiriesCopy.sq.loadError);
  listResponse = () => json({ requests: [], nextCursor: null });
  page.querySelector<HTMLButtonElement>('[data-inquiries-retry]')!.click();
  await vi.waitFor(() => expect(service.state()).toBe('ready')); await fixture.whenStable();
  expect(page.querySelector('[data-inquiries-empty]')?.textContent).toContain(inquiriesCopy.sq.emptyTitle);
  expect(page.querySelectorAll('[data-inquiry-card]')).toHaveLength(0);
  listResponse = () => json(pageData); service.reload();
  await vi.waitFor(() => expect(service.state()).toBe('ready')); await fixture.whenStable();
  detailResponse = () => json({}, 404);
  page.querySelector<HTMLButtonElement>('[data-inquiry-view]')!.click();
  await vi.waitFor(() => expect(service.detailState()).toBe('missing')); await fixture.whenStable();
  expect(page.querySelector('main [role="alert"]')?.textContent).toContain(inquiriesCopy.sq.missing);
  current = null; TestBed.inject(AccountSessionService).invalidate(); await fixture.whenStable();
  expect(page.querySelectorAll('[data-inquiry-card]')).toHaveLength(0);
  expect(page.querySelector('[data-inquiries-login]')?.getAttribute('href')).toBe('/auth/login?returnTo=%2Fsq%2Finquiries');
  expect(page.textContent).not.toContain('PRIVATE-SYMPTOM');
  expect(page.querySelector('[data-inquiries-login-help]')?.textContent).toContain(inquiriesCopy.sq.expired);
  TestBed.inject(AccountSessionService).loginAvailable.set(false); await fixture.whenStable();
...
it('preserves overview context on language change and clears private data during a real shared-service logout', async () => {
  ...
  expect(TestBed.inject(LanguageService).switchUrl('en')).toBe('/en/inquiries?view=history#inquiries-title');
  await TestBed.inject(Router).navigateByUrl('/en/inquiries'); await fixture.whenStable();
  expect(page.querySelector('h1')?.textContent).toContain(inquiriesCopy.en.title);
  await TestBed.inject(AccountSessionService).logout(); await fixture.whenStable();
