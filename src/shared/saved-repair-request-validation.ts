...
function vehicle(value: unknown): boolean {
  ...
  return choice(value['makeId'], REPAIR_REQUEST_VEHICLE_MAKES) &&
    choice(value['vehicleClass'], REPAIR_REQUEST_VEHICLE_CLASSES) &&
    choice(value['fuel'], REPAIR_REQUEST_FUELS) &&
    integer(value['year'], 1886, 2100) && integer(value['mileageKm'], 0, 2_000_000) &&
...
function summary(value: unknown): value is RecordValue {
  ...
  return identifier(value['id']) &&
    ...
    text(value['symptomPreview'], 160) && vehicle(value['vehicle']) &&
    Array.isArray(value['areas']) && value['areas'].length <= 3 &&
    value['areas'].every((area: unknown) => record(area) &&
      typeof area['placeId'] === 'string' &&
      (REPAIR_REQUEST_PLACES as readonly string[]).includes(area['placeId']) &&
      typeof area['radiusKm'] === 'number' && integer(area['radiusKm'], 5, 100));
}
...
export function isRepairRequestPage(value: unknown): value is RepairRequestPage {
  ...
  return requests.length <= REPAIR_REQUEST_MAX_PAGE_LIMIT && requests.every(summary) &&
    new Set(requests.map((item: RecordValue) => item['id'])).size === requests.length &&
    (value['nextCursor'] === null || (identifier(value['nextCursor']) &&
      requests.length > 0 && value['nextCursor'] === requests[requests.length - 1]['id']));
}

export function isSavedRepairRequest(value: unknown): value is SavedRepairRequest {
  return summary(value) && text(value['symptom'], 2_000) &&
    typeof value['earliestDropoffOn'] === 'string' && isLocalCalendarDate(value['earliestDropoffOn']) &&
    typeof value['latestPickupOn'] === 'string' && isLocalCalendarDate(value['latestPickupOn']) &&
    Array.isArray(value['attachmentIds']) && value['attachmentIds'].length <= 5 &&
