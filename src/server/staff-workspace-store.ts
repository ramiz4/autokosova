...
export class PostgresStaffWorkspace {
  ...
  async assign(principal: Principal, caseId: string, input: StaffCaseAssignment): Promise<void> {
    ...
    return this.transaction(principal, async (client) => {
      ...
      if (!Number.isSafeInteger(input.revision)) throw new AccessError(422, 'A case revision is required');
      if (!activeStatuses.includes(row.status) || !['report', 'review_submission'].includes(row.kind))
      ...
      if (row.assigned_moderator_user_id === input.moderatorUserId && !row.escalation_reason) return;
      if (row.kind === 'review_submission') {
        ...
        const appeal = row.appeal_against_user_id !== null && ['published', 'rejected'].includes(state);
        ...
        if (!appeal) {
          await client.query("UPDATE garage_review SET publication_state='under_review' WHERE id=$1", [row.subject_id]);
          await client.query("UPDATE visit_evidence SET verification_state='under_review' WHERE review_id=$1", [row.subject_id]);
        }
      }
      await client.query(
  ...
  async escalate(principal: Principal, caseId: string, input: StaffCaseEscalation): Promise<void> {
    if (!STAFF_ESCALATION_REASONS.includes(input.reason)) throw new AccessError(422, 'Invalid escalation reason');
    return this.transaction(principal, async (client) => {
      const row = await this.case(client, principal, caseId, 'update');
      assertCaseRevision(row.revision, input.revision);
      if (!Number.isSafeInteger(input.revision)) throw new AccessError(422, 'A case revision is required');
      if (row.assigned_moderator_user_id !== principal.userId || row.escalation_reason || !activeStatuses.includes(row.status))
        throw new AccessError(409, 'Only a currently assigned open case can be returned to administration');
      ...
      if (row.kind === 'review_submission')
        await client.query('DELETE FROM review_moderator_assignment WHERE review_id=$1', [row.subject_id]);
      await client.query(
  ...
  private async conflict(client: pg.PoolClient, row: CaseRow, userId: string): Promise<boolean> {
    ...
    if (row.subject_type === 'review') {
      const author = await client.query('SELECT 1 FROM garage_review WHERE id=$1 AND author_user_id=$2', [row.subject_id, userId]);
      return Boolean(author.rowCount);
    }
    return false;
  }

  private async audit(client: pg.PoolClient, actor: string, id: string, action: string): Promise<void> {
    await client.query(
  ...
  private async transaction<T>(principal: Principal, run: (client: pg.PoolClient) => Promise<T>): Promise<T> {
    if (!principal.roles.has('admin') && !principal.roles.has('moderator')) throw new AccessError(403, 'Staff access denied');
    const client = await this.pool.connect();
    try {
      await client.query('BEGIN');
      await client.query("SELECT set_config('app.user_id',$1,true),set_config('app.system_role',$2,true)", [principal.userId, principal.roles.has('admin') ? 'admin' : 'moderator']);
...
function summary(row: CaseRow): StaffCaseSummary {
  return {
    ...
    label: row.label ?? '',
    ...(row.assigned_moderator_user_id ? { assignedModeratorUserId: row.assigned_moderator_user_id } : {}),
    ...(row.assigned_label ? { assignedModeratorLabel: row.assigned_label } : {}),
    ...(row.reason_code ? { reasonCode: row.reason_code } : {}),
    ...(row.escalation_reason && row.escalated_at ? { escalation: { reason: row.escalation_reason, createdAt: row.escalated_at.toISOString() } } : {}),
  };
}
