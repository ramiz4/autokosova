...
const input: RepairRequestInput = {
  areas: [{ placeId: 'xk-pristina', radiusKm: 20 }, { placeId: 'xk-peja', radiusKm: 10 }],
  earliestDropoffOn: '2026-10-02', latestPickupOn: '2026-10-06', serviceCategoryId: 'bremsen',
  symptom: 'P'.repeat(170) + 'PRIVATE-TAIL',
  vehicle: { vehicleClass: 'suv', makeId: 'skoda', model: 'Fixture', year: 2018, engineDetails: 'PRIVATE-ENGINE' },
};

test('PostgreSQL listing preserves cursor microseconds, stable ties and explicit ownership', { skip: !databaseUrl }, async () => {
  ...
  try {
    ...
    for (let i = 0; i < 4; i++) {
      const posted = await app.inject({ method: 'POST', url: '/api/me/repair-requests',
        headers: { cookie: `autokosova_session=${session.sessionId}; autokosova_csrf=${session.csrfToken}`, 'x-csrf-token': session.csrfToken },
        payload: input });
      assert.equal(posted.statusCode, 201);
      ids.push(posted.json().id);
    }
    const foreign = await store.createRepairRequest(other, { ...input, symptom: 'OTHER-ACCOUNT' });
    // Two sub-millisecond timestamps deliberately differ while both become the same JS Date.
    await pool.query("UPDATE repair_request SET created_at = '2026-09-14 12:00:00.123456+00' WHERE owner_user_id = $1", [owner]);
    await pool.query("UPDATE repair_request SET created_at = '2026-09-14 12:00:00.123457+00' WHERE id = $1", [ids[0]]);
    const expected = [ids[0], ...ids.slice(1).sort().reverse()];
    const received: string[] = [];
    let cursor: string | undefined;
    for (let i = 0; i < 4; i++) {
      const page = await store.listRepairRequests(owner, { limit: 1, ...(cursor ? { cursor } : {}) });
      ...
      for (const value of ['PRIVATE-TAIL', 'PRIVATE-ENGINE', 'OTHER-ACCOUNT', 'earliestDropoffOn', 'attachmentIds', 'ownerUserId']) assert.ok(!json.includes(value));
    ...
    for (const id of [foreign.id, 'absent']) {
      await assert.rejects(store.listRepairRequests(owner, { limit: 20, cursor: id }),
    ...
    const index = await pool.query("SELECT indexname FROM pg_indexes WHERE indexname = 'repair_request_owner_created_id_idx'");
    assert.equal(index.rowCount, 1);

    const minimal = await store.createRepairRequest(owner, { ...input, areas: [], vehicle: undefined, symptom: undefined });
    ...
    await pool.query('DELETE FROM repair_request WHERE id = $1 AND owner_user_id = $2', [minimal.id, owner]);
    await assert.rejects(store.listRepairRequests(owner, { limit: 20, cursor: minimal.id }), AccessError);
  } finally {
    await app.close();
    await pool.query('DELETE FROM request_search_area WHERE repair_request_id IN (SELECT id FROM repair_request WHERE owner_user_id = ANY($1::text[]))', [[owner, other]]);
    await pool.query('DELETE FROM repair_request WHERE owner_user_id = ANY($1::text[])', [[owner, other]]);
