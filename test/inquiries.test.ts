...
import { buildRepairRequestSearchParams, type RepairRequestInput } from '../src/shared/repair-request';

const input: RepairRequestInput = {
  serviceCategoryId: 'bremsen',
  areas: [{ placeId: 'xk-pristina', radiusKm: 20 }, { placeId: 'xk-prizren', radiusKm: 35 }],
  earliestDropoffOn: '2026-10-02', latestPickupOn: '2026-10-06',
  symptom: 'A'.repeat(170) + 'PRIVATE-TAIL',
  vehicle: { makeId: 'skoda', model: 'Fiktives Modell', year: 2018, vehicleClass: 'suv',
    engineDetails: 'PRIVATE-ENGINE', transmissionDetails: 'PRIVATE-TRANSMISSION', mileageKm: 123_456 },
};
...
test('existing POST persists into the same owner-only list and detail, including repeated reads', async () => {
  ...
  try {
    ...
    const posted = await app.inject({ method: 'POST', url: '/api/me/repair-requests',
      headers: headers(owner, true), payload: { ...input, attachmentIds: [grant.fileId] } });
    ...
    for (let reload = 0; reload < 2; reload++) {
      ...
      assert.deepEqual(list.json().requests[0].vehicle, { makeId: 'skoda', model: 'Fiktives Modell', year: 2018, vehicleClass: 'suv' });
      for (const privateValue of [grant.fileId, 'PRIVATE-TAIL', 'PRIVATE-ENGINE', 'PRIVATE-TRANSMISSION', '123456', 'inquiries-owner', 'OTHER-ACCOUNT', '2026-10-02'])
        assert.ok(!list.body.includes(privateValue));
      assert.deepEqual(Object.keys(list.json().requests[0]).sort(), ['areas', 'createdAt', 'id', 'serviceCategoryId', 'symptomPreview', 'vehicle']);
    }
    const detail = await app.inject({ url: `/api/me/repair-requests/${id}`, headers: headers(owner) });
    ...
    for (const session of [other, admin]) {
      const foreign = await app.inject({ url: `/api/me/repair-requests/${id}`, headers: headers(session) });
      const absent = await app.inject({ url: '/api/me/repair-requests/absent', headers: headers(session) });
    ...
    assert.deepEqual(Object.keys(store.getRepairRequest('inquiries-owner', id).vehicle!).sort(), Object.keys(input.vehicle!).sort());
...
test('memory pagination is bounded, deterministic at equal timestamps, owner-scoped and detached', (t) => {
  ...
  const store = new AccessStore();
  const ids = Array.from({ length: 24 }, () => store.createRepairRequest('owner', input).id).sort().reverse();
  const foreign = store.createRepairRequest('other', input);
  const first = store.listRepairRequests('owner', { limit: 20 });
  assert.deepEqual(first.requests.map((item) => item.id), ids.slice(0, 20));
  const last = store.listRepairRequests('owner', { limit: 20, cursor: first.nextCursor! });
  assert.deepEqual(last.requests.map((item) => item.id), ids.slice(20));
  ...
  for (const cursor of [foreign.id, 'deleted-or-absent']) {
    assert.throws(() => store.listRepairRequests('owner', { limit: 20, cursor }),
  ...
  const minimal = store.createRepairRequest('minimal', { ...input, vehicle: undefined, symptom: undefined, areas: [] });
...
test('newer inserts do not shift an existing keyset page; default limit and query validation are explicit', (t) => {
  ...
  assert.deepEqual(store.listRepairRequests('owner', { limit: 20, cursor: first.nextCursor! }).requests.map((item) => item.id), original.slice(1));
  assert.deepEqual(parseRepairRequestPage({}), { limit: 20 });
  for (const query of [{ owner: 'other' }, { userId: 'other' }, { limit: '51' }, { limit: '0' },
    { limit: '1.5' }, { limit: '01' }, { limit: ['1', '2'] }, { cursor: '' }, { cursor: ['x'] }, { cursor: '../x' }])
  ...
  try {
    for (const url of ['/api/me/repair-requests', '/api/me/repair-requests/any']) {
      for (const session of [undefined, expired]) {
        const response = await app.inject({ url, ...(session ? { headers: headers(session) } : {}) });
        assert.equal(response.statusCode, 401);
        privateHeaders(response);
      }
    }
    for (const query of ['userId=other', 'ownerUserId=other', 'limit=1&limit=2', 'cursor=x&cursor=y', 'limit=51']) {
      const response = await app.inject({ url: `/api/me/repair-requests?${query}`, headers: headers(active) });
...
for (const detail of [false, true]) {
  test(`session revocation during asynchronous ${detail ? 'detail' : 'list'} read discards the result`, async () => {
    ...
    try {
      const response = await app.inject({ url: '/api/me/repair-requests' + (detail ? `/${created.id}` : ''), headers: headers(session) });
      assert.equal(response.statusCode, 401);
...
test('unexpected database errors are sanitized for both private reads', async () => {
  const store = new AccessStore();
  const session = store.createSession('owner');
  const fail = () => { throw new Error('PRIVATE-DB-ROW with engine, travel and credentials'); };
  const app = createServer({ accessStore: store, repairRequestStore: {
    createRepairRequest: fail, getRepairRequest: fail, listRepairRequests: fail,
  } });
  try {
    for (const url of ['/api/me/repair-requests', '/api/me/repair-requests/any']) {
...
test('canonical pages are private/noindex, absent from sitemap, and safe login returns remain local', async () => {
  const store = new AccessStore();
  const app = createServer({ accessStore: store, publicSiteUrl: 'https://example.invalid', oidcConfig: {
    clientId: 'fixture-client', audience: 'fixture-client', issuer: 'https://identity.invalid',
    authorizationEndpoint: 'https://identity.invalid/authorize', tokenEndpoint: 'https://identity.invalid/token',
    jwksUri: 'https://identity.invalid/jwks', redirectUri: 'https://example.invalid/auth/callback',
  } });
  try {
    ...
    for (const target of [...paths, 'https://evil.invalid/inquiries', '//evil.invalid/inquiries', '/inquiries?userId=other', '/inquiries#secret', '/inquiries-elsewhere']) {
      const response = await app.inject({ url: '/auth/login?returnTo=' + encodeURIComponent(target) });
      assert.equal(response.statusCode, 302);
      const state = new URL(response.headers.location!).searchParams.get('state')!;
      assert.equal(store.consumeOidcTransaction(state)?.returnTo, paths.includes(target) ? target : '/');
    }
  } finally { await app.close(); }
});
