...
try {
  ...
  const account = (id = 'a') => ({
    userId: `fictional-owner-${id}`,
    displayName: `FIKTIVES TESTKONTO ${id}`,
    roles: ['customer'],
    garageMemberships: [],
  ...
  socket.addEventListener('message', ({ data }) => {
    ...
    if (
      ...
      event.params.request.url.includes('/api/analytics')
    )
    ...
    if (url.pathname === '/auth/logout') {
      ...
      payload = empty
        ...
        : accountPayload.userId === 'fictional-owner-b'
          ? {
              requests: [
                {
                  ...minimal,
                  id: 'fictional-request-b',
                  symptomPreview: 'FIKTIVER TESTFALL KONTO B',
                },
              ],
              nextCursor: null,
            }
          : url.searchParams.has('cursor')
            ? { requests: [minimal], nextCursor: null }
  ...
  await command('Page.addScriptToEvaluateOnNewDocument', {
    source: `if (!sessionStorage.getItem(${JSON.stringify(draftKey)})) sessionStorage.setItem(${JSON.stringify(draftKey)}, ${JSON.stringify(draft)});`,
  });
  ...
  for (const locale of ['de', 'sq', 'en']) {
    ...
    const serverPage = await fetch(origin + path, {
      headers: { cookie: 'autokosova_session=fictional-invalid-cookie' },
    });
    ...
    assert.doesNotMatch(
      await serverPage.text(),
      /fictional-request|PRIVATE-ENGINE|PRIVATE-SYMPTOM|fictional-owner/,
    );
    for (const width of [360, 390, 430, 1280]) {
      ...
      assert.deepEqual(
        await evaluate(
          `(() => { const link = document.querySelector('[data-account-inquiries]'); return [link.getAttribute('href'), link.getAttribute('aria-current'), link.classList.contains('bg-blue-50')]; })()`,
        ),
        [path, 'page', true],
      );
      ...
      await evaluate(`document.querySelector('[data-inquiry-view]').focus()`);
      await key('Enter', 13);
      await until(
        () => evaluate(`!!document.querySelector('[data-inquiry-detail]')`),
        'readonly expansion',
      );
      assert.ok(
        await evaluate(
          `!!document.querySelector('time[datetime="2026-10-02"]') && !!document.querySelector('time[datetime="2026-10-06"]')`,
        ),
      );
      ...
      await until(
        () => evaluate(`document.querySelectorAll('[data-inquiry-card]').length === 2`),
      ...
      console.log(
        `Inquiries layout, keyboard, details, privacy and pagination passed: ${locale} ${width}px (fictional fixtures)`,
      );
    }
  }
  ...
  assert.equal(
    await evaluate(`document.querySelector('[data-new-inquiry]').getAttribute('href')`),
    '/inquiry',
  );
  ...
  await until(
    () =>
      evaluate(`document.querySelector('main').textContent.includes('FIKTIVER TESTFALL KONTO B')`),
    'account switch',
  );
  assert.ok(
    !(await evaluate(`document.querySelector('main').textContent.includes('PRIVATE-SYMPTOM')`)),
  );
  accountPayload = account();
  empty = true;
  await command('Page.reload', { ignoreCache: true });
  await until(
    () => evaluate(`!!document.querySelector('[data-inquiries-empty]')`),
    'empty account',
  );
  empty = false;
  listStatus = 503;
  await command('Page.reload', { ignoreCache: true });
  await until(() => evaluate(`!!document.querySelector('[data-inquiries-retry]')`), 'list error');
  listStatus = 200;
  await evaluate(`document.querySelector('[data-inquiries-retry]').click()`);
  await until(() => evaluate(rendered), 'list retry');
  detailStatus = 503;
  ...
  detailStatus = 200;
  await evaluate(`document.querySelector('[data-detail-retry]').click()`);
  ...
  await until(
    () =>
      evaluate(
        `document.querySelector('main [role="alert"]')?.textContent.includes('nicht mehr verfügbar')`,
      ),
    'missing detail',
  );
  detailStatus = 200;
  accountPayload = { ...account(), expiresAt: new Date(Date.now() + 3000).toISOString() };
  await command('Page.reload', { ignoreCache: true });
  await until(() => evaluate(rendered), 'short-lived session');
  await until(
    () => evaluate(`!!document.querySelector('[data-inquiries-login]') && !(${rendered})`),
    'session expiry',
  );
  assert.equal(
    await evaluate(`document.querySelector('[data-inquiries-login]').getAttribute('href')`),
    '/auth/login?returnTo=%2Finquiries',
  );
  assert.ok(
    !(await evaluate(`document.querySelector('main').textContent.includes('PRIVATE-SYMPTOM')`)),
  );
  ...
  await until(
    () => evaluate(`!!document.querySelector('[data-inquiries-login]')`),
    'guest return path',
  );
  assert.equal(
    await evaluate(`document.querySelector('[data-inquiries-login]').getAttribute('href')`),
