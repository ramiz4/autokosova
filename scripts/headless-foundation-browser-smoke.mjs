...
try {
  ...
  const overlayTrigger = page.locator('[data-foundation-overlay-trigger]');
  assert.equal(await overlayTrigger.getAttribute('aria-haspopup'), null);
  await overlayTrigger.click();
  const overlayPanel = page.locator('[data-foundation-overlay-panel]');
  ...
  await assertFocused(page, '[data-foundation-overlay-trigger]');
