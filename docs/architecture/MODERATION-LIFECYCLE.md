...
Die administrative Oberfläche verwendet den vorhandenen Lifecycle-Vertrag: `/admin/privacy` zeigt zuerst begrenzte Löschaufträge, ihre **gebundene** Policy, aktuelle Blocker und ausstehende Dateilöschungen; die Konfiguration einer neuen Policy ist ein separat zu öffnender Betreiber-Schritt. Fristen sind nicht vorbelegt und bedürfen einer tatsächlichen Betreiberfreigabe-Referenz; synthetische Tests verwenden eindeutig markierte Testwerte, keine behauptete reale Freigabe. Die zuletzt konfigurierte Policy ersetzt eine gebundene Antragsversion nie stillschweigend. Nur ein noch ungebundener, wegen fehlender Policy blockierter Auftrag kann durch die bewusste Voraussetzungen-Aktualisierung gebunden werden. Der Ausführungsdialog benennt die gebundene Policyversion und Folgen. Aktive Eigentümerschaft wird unmittelbar vor Ausführung erneut geprüft, nicht nur beim Anlegen des Auftrags.

Der Lesepfad projiziert höchstens aktive Eigentümergaragen (technische ID und Name, begrenzt), Anzahl privater Dateien, Bewertungen und Meldungen. Fahrzeuge, Reparaturanfragen und Favoriten werden nur als betroffene Objektarten erklärt, nie gezählt oder gelesen. Er setzt nie `app.user_id` auf den Antragsteller und erweitert keine RLS-Policy. Nach erfolgreicher autorisierter Prüfung löscht allein der bestehende Ausführungs-Writer Anfrage-/Fahrzeug-/Favoritendaten im spezifischen Owner-Kontext des Antragstellers. Normale Adminabfragen behalten ihre owner-only RLS-Grenze. `administration-postgres.test.ts` kontrolliert mit echter nicht privilegierter Laufzeitverbindung und separater Prüfverbindung die physische Datenbankentfernung, statt Nulltreffer als erfolgreichen Löschvorgang zu missverstehen. Externe Objektlöschaufträge sind bis zur Worker-Bestätigung lediglich vorgemerkt; „abgeschlossen“ bedeutet deshalb nie, dass bereits alle physischen Dateien gelöscht sind.
