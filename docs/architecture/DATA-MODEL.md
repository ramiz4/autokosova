...
| `Garage`             | `id`, `name`, `publication_state`, `place_id`, `location_point nullable`, `location_source nullable`, `public_phone nullable`, `public_whatsapp`  | Öffentliche View nur bei `publication_state=published`. `place_id` ist die Ortszuordnung; `location_point` ist ein separat bestätigbarer Werkstattpunkt. Bestehende Ortsmittelpunkte werden nicht übernommen. `public_whatsapp=true` ist eine eigene veröffentlichte Angabe und wird nie aus einer Telefonnummer abgeleitet. Unternehmensprüfung und Reparaturqualität bleiben getrennte Aussagen. |
